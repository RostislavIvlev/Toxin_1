    ...
    $.fn.doughnut = function(data) {
        ...
        var doughnutTemplate = function(target) {
            ...
            for (let i = 0; i < transcripts.length; i++) {
                self.transcriptLine = $("<div/>").appendTo(doughnutTranscript);
                self.transcriptLine.addClass("transcriptLine");

                self.elementColor = $("<div/>").appendTo(transcriptLine);
                self.elementColor.addClass("transcriptLine__color transcriptLine__color_" + i);

                self.elementText = $("<div/>").appendTo(transcriptLine);
                self.elementText.addClass("transcriptLine__text");
        ...
        var score = function(data) {
            calcSummary(data);
            $(".doughnutWrapper__score").text(summary);
        }
